...
flowchart LR
    U[浏览器] -->|HTTPS| C[Caddy]
    C -->|127.0.0.1 HTTP| A[Go 单体应用]
    A --> S[(SQLite)]
    A --> E[嵌入模板与静态资源]
    T[systemd timer] --> B[备份命令]
    B --> L[本地滚动备份]
    B -.可选.-> P[阿里云盘 CLI]
```

应用不需要独立前端构建、缓存服务或数据库服务器。SQLite 使用 WAL、外键、繁忙等待和有限连接池。
...
- 反向代理：应用默认只监听回环地址；客户端 IP 依赖可信代理写入的 `X-Forwarded-For`。
