...
项目采用 Go 单体应用和 SQLite，页面、静态资源与数据库迁移都编译进一个可执行文件，适合小团队在低配置 VPS 上自托管。
...
```text
浏览器 ──HTTPS── Caddy ──HTTP── Go 应用 ── SQLite
                                  │
...
生产部署示例位于 [deploy](deploy/)；完整步骤见 [VPS 部署说明](docs/VPS部署说明.md)。
...
数据库、备份、导入表格、SSH 密钥、内部交接资料、真实员工照片和构建产物都由 `.gitignore` 排除。公开前仍应执行：
...
逐项确认没有姓名、邮箱、手机、项目资料、工时、密码哈希、访问令牌或生产地址。详细审计结论见 [开源前审计](docs/OPEN_SOURCE_AUDIT.md)。
...
- 阿里云盘备份依赖第三方 `tickstep/aliyunpan`，应固定版本并单独保护 OAuth 配置。
